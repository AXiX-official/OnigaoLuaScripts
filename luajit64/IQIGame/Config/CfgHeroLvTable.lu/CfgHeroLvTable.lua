-- chunkname: @IQIGame\\Config\\CfgHeroLvTable.lua

local CfgHeroLvTable = {
	{
		Id = 1,
		UpgradeNeedExp = 80,
		NeedPlayerLv = 1,
		HeroLv = 1,
		ElementReactionParam = {
			12,
			59,
			52,
			18,
			56,
			12
		}
	},
	{
		Id = 2,
		UpgradeNeedExp = 100,
		NeedPlayerLv = 1,
		HeroLv = 2,
		ElementReactionParam = {
			12,
			60,
			53,
			18,
			57,
			12
		}
	},
	{
		Id = 3,
		UpgradeNeedExp = 120,
		NeedPlayerLv = 1,
		HeroLv = 3,
		ElementReactionParam = {
			12,
			61,
			54,
			19,
			58,
			12
		}
	},
	{
		Id = 4,
		UpgradeNeedExp = 150,
		NeedPlayerLv = 1,
		HeroLv = 4,
		ElementReactionParam = {
			13,
			64,
			57,
			20,
			61,
			13
		}
	},
	{
		Id = 5,
		UpgradeNeedExp = 170,
		NeedPlayerLv = 1,
		HeroLv = 5,
		ElementReactionParam = {
			14,
			67,
			59,
			21,
			64,
			14
		}
	},
	{
		Id = 6,
		UpgradeNeedExp = 200,
		NeedPlayerLv = 1,
		HeroLv = 6,
		ElementReactionParam = {
			14,
			71,
			62,
			22,
			67,
			14
		}
	},
	{
		Id = 7,
		UpgradeNeedExp = 230,
		NeedPlayerLv = 1,
		HeroLv = 7,
		ElementReactionParam = {
			15,
			75,
			66,
			23,
			71,
			15
		}
	},
	{
		Id = 8,
		UpgradeNeedExp = 260,
		NeedPlayerLv = 1,
		HeroLv = 8,
		ElementReactionParam = {
			17,
			81,
			72,
			25,
			77,
			17
		}
	},
	{
		Id = 9,
		UpgradeNeedExp = 290,
		NeedPlayerLv = 1,
		HeroLv = 9,
		ElementReactionParam = {
			18,
			87,
			77,
			27,
			82,
			18
		}
	},
	{
		Id = 10,
		UpgradeNeedExp = 330,
		NeedPlayerLv = 1,
		HeroLv = 10,
		ElementReactionParam = {
			19,
			93,
			83,
			29,
			89,
			19
		}
	},
	{
		Id = 11,
		UpgradeNeedExp = 360,
		NeedPlayerLv = 1,
		HeroLv = 11,
		ElementReactionParam = {
			21,
			101,
			90,
			31,
			96,
			21
		}
	},
	{
		Id = 12,
		UpgradeNeedExp = 400,
		NeedPlayerLv = 1,
		HeroLv = 12,
		ElementReactionParam = {
			22,
			110,
			97,
			34,
			104,
			22
		}
	},
	{
		Id = 13,
		UpgradeNeedExp = 440,
		NeedPlayerLv = 1,
		HeroLv = 13,
		ElementReactionParam = {
			24,
			118,
			105,
			37,
			113,
			24
		}
	},
	{
		Id = 14,
		UpgradeNeedExp = 480,
		NeedPlayerLv = 1,
		HeroLv = 14,
		ElementReactionParam = {
			26,
			129,
			114,
			40,
			122,
			26
		}
	},
	{
		Id = 15,
		UpgradeNeedExp = 520,
		NeedPlayerLv = 1,
		HeroLv = 15,
		ElementReactionParam = {
			28,
			139,
			123,
			43,
			132,
			28
		}
	},
	{
		Id = 16,
		UpgradeNeedExp = 560,
		NeedPlayerLv = 1,
		HeroLv = 16,
		ElementReactionParam = {
			31,
			150,
			133,
			46,
			143,
			31
		}
	},
	{
		Id = 17,
		UpgradeNeedExp = 600,
		NeedPlayerLv = 1,
		HeroLv = 17,
		ElementReactionParam = {
			33,
			162,
			143,
			50,
			154,
			33
		}
	},
	{
		Id = 18,
		UpgradeNeedExp = 640,
		NeedPlayerLv = 1,
		HeroLv = 18,
		ElementReactionParam = {
			36,
			174,
			154,
			54,
			166,
			36
		}
	},
	{
		Id = 19,
		UpgradeNeedExp = 690,
		NeedPlayerLv = 1,
		HeroLv = 19,
		ElementReactionParam = {
			38,
			188,
			167,
			58,
			179,
			38
		}
	},
	{
		Id = 20,
		UpgradeNeedExp = 740,
		NeedPlayerLv = 1,
		HeroLv = 20,
		ElementReactionParam = {
			41,
			202,
			179,
			62,
			192,
			41
		}
	},
	{
		Id = 21,
		UpgradeNeedExp = 790,
		NeedPlayerLv = 1,
		HeroLv = 21,
		ElementReactionParam = {
			44,
			217,
			192,
			67,
			206,
			44
		}
	},
	{
		Id = 22,
		UpgradeNeedExp = 840,
		NeedPlayerLv = 1,
		HeroLv = 22,
		ElementReactionParam = {
			47,
			232,
			206,
			72,
			221,
			47
		}
	},
	{
		Id = 23,
		UpgradeNeedExp = 890,
		NeedPlayerLv = 1,
		HeroLv = 23,
		ElementReactionParam = {
			51,
			248,
			220,
			77,
			236,
			51
		}
	},
	{
		Id = 24,
		UpgradeNeedExp = 940,
		NeedPlayerLv = 1,
		HeroLv = 24,
		ElementReactionParam = {
			54,
			265,
			234,
			82,
			252,
			54
		}
	},
	{
		Id = 25,
		UpgradeNeedExp = 960,
		NeedPlayerLv = 1,
		HeroLv = 25,
		ElementReactionParam = {
			58,
			282,
			250,
			87,
			268,
			58
		}
	},
	{
		Id = 26,
		UpgradeNeedExp = 1020,
		NeedPlayerLv = 1,
		HeroLv = 26,
		ElementReactionParam = {
			61,
			301,
			266,
			93,
			286,
			61
		}
	},
	{
		Id = 27,
		UpgradeNeedExp = 1080,
		NeedPlayerLv = 1,
		HeroLv = 27,
		ElementReactionParam = {
			65,
			320,
			283,
			99,
			304,
			65
		}
	},
	{
		Id = 28,
		UpgradeNeedExp = 1150,
		NeedPlayerLv = 1,
		HeroLv = 28,
		ElementReactionParam = {
			69,
			339,
			300,
			105,
			322,
			69
		}
	},
	{
		Id = 29,
		UpgradeNeedExp = 1210,
		NeedPlayerLv = 1,
		HeroLv = 29,
		ElementReactionParam = {
			73,
			359,
			318,
			111,
			342,
			73
		}
	},
	{
		Id = 30,
		UpgradeNeedExp = 1280,
		NeedPlayerLv = 1,
		HeroLv = 30,
		ElementReactionParam = {
			78,
			381,
			337,
			118,
			362,
			78
		}
	},
	{
		Id = 31,
		UpgradeNeedExp = 1350,
		NeedPlayerLv = 1,
		HeroLv = 31,
		ElementReactionParam = {
			82,
			402,
			356,
			124,
			382,
			82
		}
	},
	{
		Id = 32,
		UpgradeNeedExp = 1420,
		NeedPlayerLv = 1,
		HeroLv = 32,
		ElementReactionParam = {
			87,
			424,
			376,
			131,
			403,
			87
		}
	},
	{
		Id = 33,
		UpgradeNeedExp = 1500,
		NeedPlayerLv = 1,
		HeroLv = 33,
		ElementReactionParam = {
			91,
			448,
			396,
			138,
			426,
			91
		}
	},
	{
		Id = 34,
		UpgradeNeedExp = 1570,
		NeedPlayerLv = 1,
		HeroLv = 34,
		ElementReactionParam = {
			96,
			471,
			417,
			146,
			448,
			96
		}
	},
	{
		Id = 35,
		UpgradeNeedExp = 1650,
		NeedPlayerLv = 1,
		HeroLv = 35,
		ElementReactionParam = {
			101,
			495,
			439,
			153,
			471,
			101
		}
	},
	{
		Id = 36,
		UpgradeNeedExp = 1730,
		NeedPlayerLv = 1,
		HeroLv = 36,
		ElementReactionParam = {
			106,
			520,
			461,
			161,
			495,
			106
		}
	},
	{
		Id = 37,
		UpgradeNeedExp = 1810,
		NeedPlayerLv = 1,
		HeroLv = 37,
		ElementReactionParam = {
			112,
			547,
			484,
			169,
			520,
			112
		}
	},
	{
		Id = 38,
		UpgradeNeedExp = 1890,
		NeedPlayerLv = 1,
		HeroLv = 38,
		ElementReactionParam = {
			117,
			573,
			508,
			177,
			545,
			117
		}
	},
	{
		Id = 39,
		UpgradeNeedExp = 1980,
		NeedPlayerLv = 1,
		HeroLv = 39,
		ElementReactionParam = {
			122,
			600,
			531,
			185,
			570,
			122
		}
	},
	{
		Id = 40,
		UpgradeNeedExp = 3000,
		NeedPlayerLv = 1,
		HeroLv = 40,
		ElementReactionParam = {
			128,
			628,
			556,
			194,
			597,
			128
		}
	},
	{
		Id = 41,
		UpgradeNeedExp = 3170,
		NeedPlayerLv = 1,
		HeroLv = 41,
		ElementReactionParam = {
			134,
			656,
			581,
			203,
			624,
			134
		}
	},
	{
		Id = 42,
		UpgradeNeedExp = 3330,
		NeedPlayerLv = 1,
		HeroLv = 42,
		ElementReactionParam = {
			140,
			686,
			607,
			212,
			652,
			140
		}
	},
	{
		Id = 43,
		UpgradeNeedExp = 3510,
		NeedPlayerLv = 1,
		HeroLv = 43,
		ElementReactionParam = {
			146,
			715,
			633,
			221,
			680,
			146
		}
	},
	{
		Id = 44,
		UpgradeNeedExp = 3680,
		NeedPlayerLv = 1,
		HeroLv = 44,
		ElementReactionParam = {
			152,
			746,
			661,
			230,
			709,
			152
		}
	},
	{
		Id = 45,
		UpgradeNeedExp = 3870,
		NeedPlayerLv = 1,
		HeroLv = 45,
		ElementReactionParam = {
			159,
			777,
			688,
			240,
			739,
			159
		}
	},
	{
		Id = 46,
		UpgradeNeedExp = 4060,
		NeedPlayerLv = 1,
		HeroLv = 46,
		ElementReactionParam = {
			165,
			809,
			717,
			250,
			770,
			165
		}
	},
	{
		Id = 47,
		UpgradeNeedExp = 4250,
		NeedPlayerLv = 1,
		HeroLv = 47,
		ElementReactionParam = {
			172,
			842,
			745,
			260,
			800,
			172
		}
	},
	{
		Id = 48,
		UpgradeNeedExp = 4450,
		NeedPlayerLv = 1,
		HeroLv = 48,
		ElementReactionParam = {
			179,
			875,
			775,
			270,
			832,
			179
		}
	},
	{
		Id = 49,
		UpgradeNeedExp = 4660,
		NeedPlayerLv = 1,
		HeroLv = 49,
		ElementReactionParam = {
			185,
			908,
			805,
			281,
			864,
			185
		}
	},
	{
		Id = 50,
		UpgradeNeedExp = 4870,
		NeedPlayerLv = 1,
		HeroLv = 50,
		ElementReactionParam = {
			193,
			944,
			836,
			291,
			898,
			193
		}
	},
	{
		Id = 51,
		UpgradeNeedExp = 5090,
		NeedPlayerLv = 1,
		HeroLv = 51,
		ElementReactionParam = {
			200,
			978,
			866,
			302,
			930,
			200
		}
	},
	{
		Id = 52,
		UpgradeNeedExp = 5310,
		NeedPlayerLv = 1,
		HeroLv = 52,
		ElementReactionParam = {
			207,
			1014,
			898,
			313,
			965,
			207
		}
	},
	{
		Id = 53,
		UpgradeNeedExp = 5540,
		NeedPlayerLv = 1,
		HeroLv = 53,
		ElementReactionParam = {
			215,
			1051,
			931,
			325,
			1000,
			215
		}
	},
	{
		Id = 54,
		UpgradeNeedExp = 5780,
		NeedPlayerLv = 1,
		HeroLv = 54,
		ElementReactionParam = {
			222,
			1089,
			964,
			336,
			1035,
			222
		}
	},
	{
		Id = 55,
		UpgradeNeedExp = 6020,
		NeedPlayerLv = 1,
		HeroLv = 55,
		ElementReactionParam = {
			230,
			1126,
			997,
			348,
			1071,
			230
		}
	},
	{
		Id = 56,
		UpgradeNeedExp = 6270,
		NeedPlayerLv = 1,
		HeroLv = 56,
		ElementReactionParam = {
			238,
			1165,
			1032,
			360,
			1108,
			238
		}
	},
	{
		Id = 57,
		UpgradeNeedExp = 6520,
		NeedPlayerLv = 1,
		HeroLv = 57,
		ElementReactionParam = {
			246,
			1204,
			1066,
			372,
			1145,
			246
		}
	},
	{
		Id = 58,
		UpgradeNeedExp = 6780,
		NeedPlayerLv = 1,
		HeroLv = 58,
		ElementReactionParam = {
			254,
			1244,
			1102,
			384,
			1183,
			254
		}
	},
	{
		Id = 59,
		UpgradeNeedExp = 7050,
		NeedPlayerLv = 1,
		HeroLv = 59,
		ElementReactionParam = {
			262,
			1285,
			1138,
			397,
			1222,
			262
		}
	},
	{
		Id = 60,
		UpgradeNeedExp = 9130,
		NeedPlayerLv = 1,
		HeroLv = 60,
		ElementReactionParam = {
			271,
			1326,
			1174,
			410,
			1261,
			271
		}
	},
	{
		Id = 61,
		UpgradeNeedExp = 9480,
		NeedPlayerLv = 1,
		HeroLv = 61,
		ElementReactionParam = {
			279,
			1368,
			1212,
			422,
			1301,
			279
		}
	},
	{
		Id = 62,
		UpgradeNeedExp = 9840,
		NeedPlayerLv = 1,
		HeroLv = 62,
		ElementReactionParam = {
			288,
			1410,
			1249,
			436,
			1341,
			288
		}
	},
	{
		Id = 63,
		UpgradeNeedExp = 10200,
		NeedPlayerLv = 1,
		HeroLv = 63,
		ElementReactionParam = {
			297,
			1454,
			1288,
			449,
			1383,
			297
		}
	},
	{
		Id = 64,
		UpgradeNeedExp = 10580,
		NeedPlayerLv = 1,
		HeroLv = 64,
		ElementReactionParam = {
			306,
			1498,
			1327,
			463,
			1425,
			306
		}
	},
	{
		Id = 65,
		UpgradeNeedExp = 10960,
		NeedPlayerLv = 1,
		HeroLv = 65,
		ElementReactionParam = {
			315,
			1542,
			1366,
			476,
			1467,
			315
		}
	},
	{
		Id = 66,
		UpgradeNeedExp = 11350,
		NeedPlayerLv = 1,
		HeroLv = 66,
		ElementReactionParam = {
			324,
			1588,
			1406,
			490,
			1510,
			324
		}
	},
	{
		Id = 67,
		UpgradeNeedExp = 11750,
		NeedPlayerLv = 1,
		HeroLv = 67,
		ElementReactionParam = {
			333,
			1634,
			1447,
			505,
			1554,
			333
		}
	},
	{
		Id = 68,
		UpgradeNeedExp = 12150,
		NeedPlayerLv = 1,
		HeroLv = 68,
		ElementReactionParam = {
			343,
			1680,
			1488,
			519,
			1598,
			343
		}
	},
	{
		Id = 69,
		UpgradeNeedExp = 12570,
		NeedPlayerLv = 1,
		HeroLv = 69,
		ElementReactionParam = {
			353,
			1727,
			1530,
			533,
			1643,
			353
		}
	},
	{
		Id = 70,
		UpgradeNeedExp = 21420,
		NeedPlayerLv = 1,
		HeroLv = 70,
		ElementReactionParam = {
			362,
			1775,
			1572,
			548,
			1688,
			362
		}
	},
	{
		Id = 71,
		UpgradeNeedExp = 22140,
		NeedPlayerLv = 1,
		HeroLv = 71,
		ElementReactionParam = {
			372,
			1824,
			1615,
			563,
			1734,
			372
		}
	},
	{
		Id = 72,
		UpgradeNeedExp = 22880,
		NeedPlayerLv = 1,
		HeroLv = 72,
		ElementReactionParam = {
			382,
			1873,
			1659,
			578,
			1781,
			382
		}
	},
	{
		Id = 73,
		UpgradeNeedExp = 23630,
		NeedPlayerLv = 1,
		HeroLv = 73,
		ElementReactionParam = {
			392,
			1923,
			1703,
			594,
			1829,
			392
		}
	},
	{
		Id = 74,
		UpgradeNeedExp = 24390,
		NeedPlayerLv = 1,
		HeroLv = 74,
		ElementReactionParam = {
			403,
			1973,
			1748,
			609,
			1877,
			403
		}
	},
	{
		Id = 75,
		UpgradeNeedExp = 25170,
		NeedPlayerLv = 1,
		HeroLv = 75,
		ElementReactionParam = {
			413,
			2025,
			1794,
			625,
			1926,
			413
		}
	},
	{
		Id = 76,
		UpgradeNeedExp = 25960,
		NeedPlayerLv = 1,
		HeroLv = 76,
		ElementReactionParam = {
			424,
			2076,
			1839,
			641,
			1975,
			424
		}
	},
	{
		Id = 77,
		UpgradeNeedExp = 26770,
		NeedPlayerLv = 1,
		HeroLv = 77,
		ElementReactionParam = {
			435,
			2129,
			1886,
			658,
			2025,
			435
		}
	},
	{
		Id = 78,
		UpgradeNeedExp = 27600,
		NeedPlayerLv = 1,
		HeroLv = 78,
		ElementReactionParam = {
			445,
			2182,
			1933,
			674,
			2075,
			445
		}
	},
	{
		Id = 79,
		UpgradeNeedExp = 28440,
		NeedPlayerLv = 1,
		HeroLv = 79,
		ElementReactionParam = {
			456,
			2236,
			1980,
			691,
			2126,
			456
		}
	},
	{
		Id = 80,
		UpgradeNeedExp = 42560,
		NeedPlayerLv = 1,
		HeroLv = 80,
		ElementReactionParam = {
			467,
			2290,
			2029,
			707,
			2178,
			467
		}
	},
	{
		Id = 81,
		UpgradeNeedExp = 43840,
		NeedPlayerLv = 1,
		HeroLv = 81,
		ElementReactionParam = {
			479,
			2345,
			2077,
			724,
			2231,
			479
		}
	},
	{
		Id = 82,
		UpgradeNeedExp = 45140,
		NeedPlayerLv = 1,
		HeroLv = 82,
		ElementReactionParam = {
			490,
			2401,
			2127,
			742,
			2284,
			490
		}
	},
	{
		Id = 83,
		UpgradeNeedExp = 46470,
		NeedPlayerLv = 1,
		HeroLv = 83,
		ElementReactionParam = {
			502,
			2458,
			2177,
			759,
			2337,
			502
		}
	},
	{
		Id = 84,
		UpgradeNeedExp = 47820,
		NeedPlayerLv = 1,
		HeroLv = 84,
		ElementReactionParam = {
			513,
			2515,
			2228,
			777,
			2392,
			513
		}
	},
	{
		Id = 85,
		UpgradeNeedExp = 49190,
		NeedPlayerLv = 1,
		HeroLv = 85,
		ElementReactionParam = {
			525,
			2573,
			2279,
			795,
			2447,
			525
		}
	},
	{
		Id = 86,
		UpgradeNeedExp = 50590,
		NeedPlayerLv = 1,
		HeroLv = 86,
		ElementReactionParam = {
			537,
			2631,
			2331,
			813,
			2502,
			537
		}
	},
	{
		Id = 87,
		UpgradeNeedExp = 52010,
		NeedPlayerLv = 1,
		HeroLv = 87,
		ElementReactionParam = {
			549,
			2690,
			2383,
			831,
			2558,
			549
		}
	},
	{
		Id = 88,
		UpgradeNeedExp = 53450,
		NeedPlayerLv = 1,
		HeroLv = 88,
		ElementReactionParam = {
			561,
			2750,
			2436,
			849,
			2616,
			561
		}
	},
	{
		Id = 89,
		UpgradeNeedExp = 54920,
		NeedPlayerLv = 1,
		HeroLv = 89,
		ElementReactionParam = {
			574,
			2811,
			2489,
			868,
			2673,
			574
		}
	},
	{
		Id = 90,
		HeroLv = 90,
		NeedPlayerLv = 1,
		ElementReactionParam = {
			586,
			2872,
			2543,
			887,
			2731,
			586
		}
	}
}

setmetatable(CfgHeroLvTable, {
	__index = function(t, k)
		error("config error :not find key: [" .. tostring(k) .. "] in table: " .. "[CfgHeroLvTable]")
	end
})

local defaultMetaTable = {
	__index = {
		NeedPlayerLv = 0,
		UpgradeNeedExp = 0,
		HeroLv = 0,
		ElementReactionParam = {
			0,
			0,
			0,
			0,
			0,
			0
		}
	}
}

for i, data in pairs(CfgHeroLvTable) do
	setmetatable(data, defaultMetaTable)
end

return CfgHeroLvTable
